...
ESX.RegisterServerCallback('esx_society:getEmployees', function(source, cb, society)
	if Config.EnableESXIdentity then
		...
		MySQL.Async.fetchAll('SELECT firstname, lastname, identifier, callsign, phone_number, job, job_grade FROM users WHERE job = @job ORDER BY job_grade DESC', {
			['@job'] = society
		}, function (results)
			local employees = {}
...
AddEventHandler('esx_society:setCallsign', function(identifier, callsign)
	local _source = source
	local xPlayer = ESX.GetPlayerFromId(_source)
	if callsign ~= nil then
		MySQL.Async.execute('UPDATE users SET callsign = @callsign WHERE identifier = @identifier', {
			['@callsign'] = callsign,
			['@identifier'] = identifier
