...
function OpenEmployeeList(society)
	...
	ESX.TriggerServerCallback('esx_society:getEmployees', function(employees)
		...
		local elements = {
			head = {_U('employee'),_U('grade'),_U('phonenumber'),_U('callsign'),_U('actions')},
			rows = {}
		}

		for i=1, #employees, 1 do
			local gradeLabel = (employees[i].job.grade_label == '' and employees[i].job.label or employees[i].job.grade_label)
			local callsign = employees[i].callsign
			local phone_number = employees[i].phone_number
			local jobname = employees[i].job.name
			table.insert(elements.rows, {
				...
				cols = {
					...
					phone_number,
                                        callsign,
					'{{' .. _U('promote') .. '|promote}} {{' .. _U('fire') .. '|fire}} {{' .. _U('callsign') .. '|callsign}}'
				}
			})
		end
